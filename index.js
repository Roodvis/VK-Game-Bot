				...
				keyboard: Keyboard.keyboard([
					...
					[
						Keyboard.textButton({
							...
							payload: {
								command: 'вывести_номер_телефона'
							},
							color: Keyboard.SECONDARY_COLOR
						})
					],
					[
						Keyboard.textButton({
							...
							payload: {
								command: 'вывести_банковкая карта'
							},
							color: Keyboard.SECONDARY_COLOR
						})
					],
...
// Вывод на номер телефона

hearCommand('вывести_номер_телефона', async (context, next) => {
	await context.send({
		message: 'Введите: /вывод телефон "ваш номер телефона" "количество рублей" (без ковычек)',
		keyboard: Keyboard.keyboard([
			[
				Keyboard.textButton({
					label: 'Играть',
...
vk.updates.hear(/^\/вывод телефон (.+) (.+)/, (context) => {
	let mobile_number = context.$match[1];
	let money_get = context.$match[2];
	if(money_get < 1000 || money_get > 50000){
		context.send({
			message: `Минимальная сумма вывода: 1000 рублей.
		...
		con.query(get_money_sql, [vk_id], function(err, result){
			if(result[0].money < money_get){
				...
				if(money_get == money_getInt){
					...
					random_id: Math.random() * 10000000000,
					message: `Поступил новый запрос на получение денег:
							  Способ оплаты: Номер телефона
							  Номер телефона: ` + mobile_number +
							 `\nКоличество денег: ` + money_get + " рублей" +
							 `\nID человека: ` + context.senderId
					});
					context.send({
...
hearCommand('вывести_банковкая карта', async (context, next) => {
	await context.send({
		message: 'Введите: /вывод банк "номер банковской карты" "количество рублей" (без ковычек)',
		keyboard: Keyboard.keyboard([
			[
				Keyboard.textButton({
					label: 'Играть',
...
vk.updates.hear(/^\/вывод банк (.+) (.+)/, (context) => {
	let card_number = context.$match[1];
	let money_get = context.$match[2];
	if(money_get < 1000 || money_get > 50000){
		context.send({
			message: `Минимальная сумма вывода: 1000 рублей.
